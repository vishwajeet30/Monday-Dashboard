...
/*boolean isUrl_exist(String file_name)
{
	...
	{	
		...
		java.net.URL url = new java.net.URL("http://192.168.62.201:8889/Imm/vishwajeet/flight_pic.jpg/" + file_name + ".jpg");
	...
	//http://192.168.62.201:8889/Imm/apis_manifest/im_apis_status_html.jsp
...
<script>
	...
	{
		document.Arrival_form.flight_number.value = "";
		document.Arrival_form.source_port1.value = "";
		document.Arrival_form.source_country1.value = "";
		document.Arrival_form.dest_port1.value = "";
		document.Arrival_form.dest_country1.value = "";

		document.Arrival_form.source_port.value = "";
		document.Arrival_form.source_country.value = "";	
		document.Arrival_form.dest_port.value = "";	
		document.Arrival_form.dest_country.value = "";	
	
		document.Arrival_form.action = "im_flight_routes_dashboard_01_arr.jsp";
		document.Arrival_form.submit();	
	}
...
<% String heading = "International Flights Arriving to India"; %>
<br>
<div id="loading" name="loading/" style="background-color: #f5f9ff; opacity: 2;" >
	<br><br>
...
<!--<body style="background-color: #e5f6ff; "> -->
	...
	<h1 style="font-family: Arial;background-color: #D0DDEA; color: #347FAA; "><center>International Flights Arriving to India</center></h1><br><br>
	<!--http://192.168.62.201:8889/Imm/vishwajeet/im_flight_routes_dashboard_01.jsp-->
	...
	String dest_port_Input = request.getParameter("dest_port") == null ? "" : request.getParameter("dest_port");
	...
	%>
	<form action="#" method="post" name="Arrival_form">
	<table align="center" width="100%" border="0" cellspacing="0" cellpadding="2" bordercolorlight="#FF99CC" 
		...
		<tr><td>Flight No.</td><td>Source Port</td><td>Source Country</td><td>Destination Port</td><td colspan="2">Destination Country</td>				
		</tr>
		<tr>
			<td>
		...
		<td>
       			<input type="text" style="color:white;font-weight:bold;height: 22px;font-size=12pt; background-color: #007d79;text-transform:uppercase;font-family:Verdana" size="2" maxlength="10" name="source_country1" onkeyup="filtery(this.value,this.form.source_country)" onchange="filtery(this.value,this.form.source_country)" onKeyDown="if(event.keyCode==13) event.keyCode=9;if (event.keyCode==8) event.keyCode=37+46;" onKeyPress="return letternumber(event, 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ')">

				<select name="source_country" style="width:100px;font-family: courier;font-size: 14px;color: blue; font-weight:bold;" onKeyDown="if(event.keyCode==13) event.keyCode=9;" >
           	<option value="" selected>Select</option>
           	    <%
		...
		<td>
			<input type="text" style="color:white;font-weight:bold;height: 22px;font-size=12pt;text-transform:uppercase;background-color: #007d79; font-family:Verdana" size="2" maxlength="10" name="dest_port1" onkeyup="filtery(this.value,this.form.dest_port)" onchange="filtery(this.value,this.form.dest_port)" onKeyDown="if(event.keyCode==13) event.keyCode=9;if (event.keyCode==8) event.keyCode=37+46;" onKeyPress="return letternumber(event, 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ')">
			<select name="dest_port" style="width:100px;font-family: courier;font-size: 14px;color: blue; font-weight:bold;" onKeyDown="if(event.keyCode==13) event.keyCode=9;" >
           	<option value="" selected>Select</option>
           	    <% 
				 try
				{				
				 ...
				 while(rsTemp.next())
				 {
					...
					%>
				 <option value="<%=rsTemp.getString("port_code")%>" <%if(dest_port_Input.equals(rsTemp.getString("port_code"))){%>Selected<%}%> ><%=rsTemp.getString("port_code")+" - "+rsTemp.getString("port_desc")%></option>
				 <%
			...
			<td>
				...
				<select name="dest_country" style="width:100px;font-family: courier;font-size: 14px;color: blue; font-weight:bold;" onKeyDown="if(event.keyCode==13) event.keyCode=9;" >
           	<option value="" selected>Select</option>
				 <%
			...
			<td>
				<button type="submit" value="Submit" style="background-color: #004144;color: white;border: none;font-weight: bold; padding: 10px 20px;text-align: center;text-decoration: none;display: inline-block;font-size: 14px; margin: 4px 2px; cursor: pointer;">Submit</button>
...
try{
	...
	if(!dest_port_Input.equals(""))
	{
		selectQuery.append(" and DEST_PORT_CODE = '"+dest_port_Input+"'");
		lblQuery = lblQuery + ", Destination Port = " + dest_port_Input.toUpperCase();
	}
...
%><h3 style="font-family: Arial; color: #fa4d56; text-align: left;font-size: 12pt; background-color: #d0e2ff;"><%=lblQuery%></h3><%
	...
	rsMain = psMain.executeQuery();  
		...
		bodercolordark="#FF99CC" bodercolor="#FF99CC" style="border-collapse: collapse;background-color: #FFFFFF; font-family: verdana;font-size: 8pt;text-align: left; border-color:#e5f6ff;">
			<tr style=" background-color: #78a9ff;font-family: Verdana; font-size: 17px; color:white;font-weight: bold;">
				...
				<td colspan="5" bgcolor="#005d5d"><center>Departure Flights</center></td>
			</tr>
			...
			<%															
				...
				if(flightNumber1.contains("-"))
					...
					%>	
						...
						<td align= "centsr" bgcolor="#3ddbd9">##</td>
